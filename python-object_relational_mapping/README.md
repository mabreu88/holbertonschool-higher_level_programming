...
<em>Wait, do you remember the previous task? Did you test</em> `"Arizona'; TRUNCATE TABLE states ; SELECT * FROM states WHERE name = '"` <em>as an input? </em>
...
Yes, it’s an `SQL injection` to delete all records of a table… </em>
...
* Your script should connect to a MySQL server running on `localhost` at port `3306`
* <strong>WARNING:</strong> all classes who inherit from `Base` <strong>must</strong> be imported before calling `Base.metadata.create_all(engine)`
...
* Your script should take 3 arguments: `mysql username`, `mysql password` and `database name`
* You must use the module `SQLAlchemy`
* You must import `State` and `Base` from `model_state` - f`rom model_state import Base, State`
* Your script should connect to a MySQL server running on `localhost` at port `3306`
